...
test.describe('Home', () => {
  ...
  /**
   * TC02: Home page - initial render shows main heading, filter, show archived checkbox, export buttons disabled, and navigation link
   */
  test('TC02 - Home page - initial render shows main heading, filter, show archived checkbox, export buttons disabled, and navigation link', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByRole('heading', { name: 'Habit Tracker' })).toBeVisible();
  ...
  /**
   * TC03: Home - toggling show archived checkbox updates displayed habits accordingly
   */
  test('TC03 - Home - toggling show archived checkbox updates displayed habits accordingly', async ({ page }) => {
    await page.goto('/');
    const showArchivedCheckbox = page.getByLabel('Show archived');
  ...
  /**
   * TC04: LogoutButton - logout button logs out and navigates to /login
   */
  test('TC04 - LogoutButton - logout button logs out and navigates to /login', async ({ page }) => {
    await page.goto('/');
  ...
  /**
   * TC05: Home - toggles show archived checkbox updates habits list
   */
  test('TC05 - Home - toggles show archived checkbox updates habits list', async ({ page }) => {
    await page.goto('/');
