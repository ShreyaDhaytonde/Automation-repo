...
Locator: getByRole('listitem').filter({ hasText: 'Test Unarchive 1789749027758' })
Expected: visible
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" getByRole('listitem').filter({ hasText: 'Test Unarchive 1789749027758' }) with timeout 10000ms
  - waiting for getByRole('listitem').filter({ hasText: 'Test Unarchive 1789749027758' })
...
```ts
  ...
  60  |     await habitCard.getByRole('button', { name: `Archive ${uniqueName}`, exact: true }).click();
...
> 63  |     await expect(archivedHabitCard).toBeVisible();
  ...
  65  |     await expect(page.getByRole('listitem').filter({ hasText: uniqueName })).toHaveCount(0);
  66  |   });
  67  | 
  68  |   /**
