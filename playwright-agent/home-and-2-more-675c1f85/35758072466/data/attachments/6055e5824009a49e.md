...
Locator: getByRole('listitem').filter({ hasText: 'CompleteAll1 1790098809548' }).getByRole('button', { name: 'Done today' })
Expected: visible
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" getByRole('listitem').filter({ hasText: 'CompleteAll1 1790098809548' }).getByRole('button', { name: 'Done today' }) with timeout 10000ms
  - waiting for getByRole('listitem').filter({ hasText: 'CompleteAll1 1790098809548' }).getByRole('button', { name: 'Done today' })
...
- main:
  ...
  - button "Export JSON"
  - button "Export CSV"
  - paragraph: Completed 1/2 habits — 1 failed. Try again.
- alert
```
